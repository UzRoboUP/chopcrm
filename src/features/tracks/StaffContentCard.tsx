...
import { useStaffDelete } from '../authentication/useStaffDelete';
...
function StaffContentCard({ item, pagename, onEdit }: ContentCardProps) {
  ...
  const { deleteStaff, isLoadingDelete } = useStaffDelete();

  const handleDelete = () => {
    deleteStaff(item.id, {
      onSuccess: (data) => {
        queryClient.setQueryData(['staffDelete'], data);
          ...
          <div className="card__items">
            ...
            <div className="card__item card__item--comment">
              ...
              <div className="card__item--value">
                {item?.staff_comment || 'без комментариев'}
              </div>
            </div>
          </div>
        </div>
