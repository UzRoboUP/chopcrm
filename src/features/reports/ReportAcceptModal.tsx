...
export default function ReportAcceptModal({
  ...
  driverName: string;
  isLoading: boolean;
  updateReportPhotoStatus: (status: string) => void;
  photo_control: {
    updated_at: string;
      ...
      {status === 'photo_report_sent' ? (
        <div className="d-flex justify-center mt-10">
          <button
            ...
            onClick={() => updateReportPhotoStatus('photo_report_rejected')}
            disabled={isLoading}
          >
            Отклонить
          </button>
          <button
            ...
            onClick={() => updateReportPhotoStatus('confirmed')}
          >
            Подтвердить
          </button>
        </div>
