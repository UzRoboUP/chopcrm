...
function StockEmployeesCard({ item, pagename, onEdit }: ContentCardProps) {
  const queryClient = useQueryClient();

  const [isOpenMenu, setOpenMenu] = useState(false);
  const [popconfirmOpen, setPopconfirmOpen] = useState(false);
  const [isOpenCommentModal, setOpenCommentModal] = useState(false);
  const { deleteStock, isLoadingDelete } = useDeleteStock();
        ...
        <div className="content__card card">
          ...
          <div className="card__items">
            ...
            <div className="card__item card__item--comment">
              ...
              <div className="card__item--value">
                {item?.stock_comment || 'без комментариев'}
              </div>
            </div>
...
export default StockEmployeesCard;
